...
REPO_PATH="${1:-.}"
CANVAS="$SCRIPT_DIR/git_dashboard.txt"
INTERVAL=5
...
# Create canvas
$CLI create --width 250 --height 120 "$CANVAS"
...
while true; do
    ...
    eval $CLI update "$CANVAS" 2 $commit_lines
    ...
    eval $CLI update "$CANVAS" 3 $branch_lines
    ...
    eval $CLI update "$CANVAS" 4 $status_lines

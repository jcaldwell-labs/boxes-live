...
echo -e "${GREEN}🚀 Launching Campaign Session: ${SESSION_NAME}${NC}"
...
if [ ! -f "${CANVAS_FILE}" ]; then
    echo -e "${BLUE}📝 Initializing realm canvas...${NC}"
    cat > "${CANVAS_FILE}" << 'EOF'
...
echo -e "${BLUE}🎭 Creating tmux session with campaign layout...${NC}"
...
tmux send-keys -t "${SESSION_NAME}:0.1" "echo -e '${GREEN}📖 NARRATIVE CONSOLE${NC}'" C-m
...
tmux send-keys -t "${SESSION_NAME}:0.2" "echo -e '${YELLOW}⚙️  STATE MANAGER${NC}'" C-m
...
echo -e "${GREEN}✓ Campaign session created!${NC}"
echo ""
